...
def test(r, Va, Vb, Vmax, Am):
    Vm_initial = r / Vmax * Am

    # Решение уравнения
    time_initial = time()
    short_solution = root(short_dist, Vm_initial, (r, Am, Va, Vb, Vmax))
    time_final = time()
    ...
    if short_solution.success:
        ...
        if np.linalg.norm(Vm) > Vmax or not short_solution.success:
            ...
            time_initial = time()
            long_solution = root(long_dist, ang_initial, (r, Am, Va, Vb, Vmax))
            time_final = time()
    ...
    if Vm_final is not None:
        ...
        t2 = np.linalg.norm(
            r - ((Vm_final + Va) * np.linalg.norm(Vm_final - Va) + (Vm_final + Vb) * np.linalg.norm(Vm_final - Vb)) / (
                    2 * Am)) / Vmax + t1
        ...
        #         now_r = r - (Vm_final + Vb) * np.linalg.norm(Vm_final - Vb) / (2 * Am) + Vm_final * (t - t2) + a * (
        #                 t - t2) ** 2 / 2
        #     way.append(now_r)
        # way.append(r)
        # plot_way(way)
...
test(np.array([9, 1]), np.array([0, -1]), np.array([0, -1]), 1.3, 6)